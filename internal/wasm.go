...
func RunWASMTests(rpcAddrs []string, log logging.Logger) {
	...
	txSendBytes, err := hex.DecodeString(txSend)
	if err != nil {
		log.Fatal("error decoding hex: %v", zap.Error(err))
		return
	}
	res, err := c.BroadcastTxSync(context.Background(), txSendBytes)
	if err != nil {
...
func GetBalances(c *rpchttp.HTTP, log logging.Logger, address, querystring string) {
	...
	// balances is not unmarshalled, it is a byte array
	// cant unmarshal it to a struct, it needed cosmos sdk dependencies
